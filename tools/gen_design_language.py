...
def _font_sizes() -> dict[str, int]:
    """fontinstall.py:FONTS as role -> Kodi <size>."""
    fonts = {
        role: int(size)
        for role, size in re.findall(
...
def _verify_against_sources(html: str) -> None:
    ...
    # Type scale: role -> pt size, as stated in the page's TYPE SCALE array.
    stated_type = {
        role: int(size)
        for role, size in re.findall(
            r"\['(tofa_font_\w+)', '[^']*', (\d+),", html
        )
    }
    if not stated_type:
        problems.append("type scale array not found -- the regex needs updating")
    real_fonts = _font_sizes()
    for role, size in sorted(stated_type.items()):
        real = real_fonts.get(role)
        if real is None:
            problems.append(f"type scale lists {role}, which fontinstall.py no longer defines")
        elif real != size:
            problems.append(f"type scale says {role} is {size}pt; fontinstall.py says {real}pt")
    missing = [
        r for r in real_fonts
        if r not in stated_type and "icons" not in r
    ]
    if missing:
        problems.append(f"type scale omits text roles fontinstall.py defines: {missing}")
...
<div class="shell">
  ...
  <main class="main">
    ...
    <section class="doc" id="tokens">
      ...
      <div class="comp">
        <div class="comp-head"><h3>Text tiers (white-alpha)</h3><span class="comp-src">theme.py:TEXT_PRIMARY/TEXT_SECONDARY/TEXT_TERTIARY</span></div>
        <p class="comp-desc">RESOLVED (2026-07-30). Spec &sect;2 defines 4 tiers (100/62/42/24%); the codebase had drifted to ~18 distinct alpha values for text. Consolidated to 3 (not spec's 4 -- "muted" 24% had no real callers once the other 3 were fixed), each confirmed by pixel-sampling real Apple TV reference captures rather than copied from the spec sight-unseen: alpha = (rendered&minus;background)/(255&minus;background) against the measured local background, for known primary/secondary/tertiary text elements. Every one of the ~219 <code>&lt;textcolor&gt;</code> attributes across the whole app (generated pipeline AND the 4 hand-authored screens) now references a Window.Property (<code>text_primary</code>/<code>text_secondary</code>/<code>text_tertiary</code>) instead of a raw hex literal -- zero literal textcolor values remain.</p>
        <div class="swatch-strip">
      ...
      <div class="comp">
        ...
        <p class="comp-desc">One family (Inter Tight) at every role, plus Roboto Mono for tabular numerals (ratings, PIN digits, device codes) and a Lucide icon-font role per pixel footprint actually used. Inter Tight's own vertical metrics run generous, so every Kodi <code class="mono">&lt;size&gt;</code> here is ~1.75&times; the nominal spec point size (measured against a real Android TV capture, see fontinstall.py's own changelog) &mdash; sizes below are Kodi's actual injected values, already carrying that correction.</p>
...
].forEach(([role, family, size, weight, sample]) => {{
  const el = document.createElement('div');
  el.className = 'type-row';
  const mono = family.startsWith('Roboto');
  el.innerHTML = `<div class="type-meta"><b>${{role}}</b>${{family}} &middot; ${{size}}pt</div><div class="type-sample" style="font-family:'${{mono?'Roboto Mono':'Inter Tight'}}';font-weight:${{weight}};font-size:${{Math.round(size*0.62)}}px">${{sample}}</div>`;
