  ...
  <main class="main">
    ...
    <section class="doc" id="tokens">
      ...
      <div class="comp">
        ...
        <p class="comp-desc">Inter Tight carries every text role, in three weights &mdash; and <b>weight is a property of the file, not a tag</b>: Kodi's <code class="mono">&lt;font&gt;</code> has no weight attribute, so "SemiBold" means the role's declaration names <code class="mono">tofa_inter_tight_semibold.ttf</code>. Since add-on 0.9.26 the files ship in <code class="mono">resource.font.tofa</code> and Kodi resolves them from there; fontinstall.py still injects the <i>declarations</i> into the active skin, but copies no file. Three roles are Roboto Mono, and only three: sign-in's <code class="mono">tofa_font_link</code> and <code class="mono">tofa_font_code</code> (a pairing code has to survive being read aloud off a TV), and <code class="mono">tofa_font_stats_value</code> in the player's &sect;8.11 stats readout, where the numbers tick once a second and a proportional face would shuffle the whole row sideways every time a digit changed width. Ratings and PIN digits are <i>not</i> mono &mdash; the rating badge is <code class="mono">tofa_font_micro</code> and the PIN pad is <code class="mono">tofa_font_button</code>. The eight Lucide roles at the bottom are one per pixel footprint the UI actually draws at.</p>
        ...
        <p class="comp-desc"><b>&sect;3's own mapping is not exempt either.</b> The hero was 77 by that &times;1.7 rule and had never been measured, because the app draws the title's LOGO there and only falls back to text when a title has none &mdash; a case no stored capture contained. Captured live on 2026-08-19 and it read 30% over: the Home fallback caps at 43px (Inter Tight 59), Detail's at 46px (63). What makes it interesting is that the SAME 77 measured exactly right on the server picker, whose heading caps at 56px &mdash; so the number was never wrong, it was wrong for two of its three callers. It now splits: <code class="mono">tofa_font_hero</code> keeps 77 and heads a screen, <code class="mono">tofa_font_hero_title</code> is 61 and stands in for artwork.</p>
        <p class="comp-desc">So: <b>sizes below are Kodi's actual injected values</b>, read out of <code class="mono">FONTS</code> at generation time. Don't back-derive a spec number from one by dividing &mdash; and don't assume a role that was derived rather than measured is right just because it has shipped for months.</p>
...
[
  {{
    ...
    props: [['Cell / poster','{cell_w}&times;{cell_h} / {poster_w}&times;{poster_h}'],['Mask radius','14px (poster-mask.png) &mdash; &sect;4's radius for the TV poster cards, and 7.9.3 states it outright; it was a hand-fitted 16 before that'],['Rating badge','52&times;28, {badge_scrim} fill + 1px {border} outline'],['Progress fill','accent, flush with poster bottom, pre-rendered % strip'],['Focus glow','card-glow.png, accent, 10px bleed all sides, 104.5% zoom'],['Caption','title tofa_font_poster_title / text_primary; meta tofa_font_metadata / text_secondary. NEITHER changes tier on focus &mdash; the meta line used to rest at text_tertiary and lift, alone in the card family, and measures a flat 62-63% in both states on every reference capture.'],['Caption (trailing)','Continue Watching only, right-justified on the meta baseline: tofa_font_micro / text_tertiary, bottom-aligned so the two sizes share a baseline. &sect;6 asks for micro type, uppercase, at white 50% &mdash; the string is already uppercase from progress.py, and 50% is not on the tier scale, so this takes the nearest tier below rather than reintroduce a one-off alpha. NB the real app renders this half at the SAME size as the year, not smaller.'],['Badge on focus','cleared on Browse/Home, KEPT on Person &mdash; both are the real app&rsquo;s own behaviour on their own screen (poster_visual(hide_rating_on_focus=))'],['Title marquee','scrolls while the LIST has focus, and only then. Two label copies with complementary Control.HasFocus gates rather than one &lt;scroll&gt;: focusedlayout renders for a list\'s active item even when the cursor is elsewhere, so an ungated marquee ran forever in the background (the first Continue Watching title scrolled while focus was still on the nav bar). Suffix is three U+2003 em spaces &mdash; Kodi strips ordinary whitespace from &lt;scrollsuffix&gt;. A title that fits does not scroll at all.']],
  }},
  {{
  ...
  {{
    ...
    props: [['Cell / thumb','350&times;284 / 330&times;186'],['Row gap','~134px art-to-art, matching Browse\'s poster grid'],['Corner badges','both inset 8 (CHIP_INSET) from the still: unaired capsule top-LEADING, watched check top-TRAILING so one episode can carry both. Different heights (24 / 28), so the shorter one\'s y is COMPUTED to keep the two centre-aligned.'],['Watched badge','28&times;28 accent circle, checkmark glyph'],['Caption','tofa_font_micro, text_secondary &mdash; same in both states'],['Title','tofa_font_poster_title, text_primary &mdash; same in both states; marquees while the grid has focus (0.9.25, same two-gated-copies technique as the poster title). Fires readily: &ldquo;Underage Drinking: A National Concern&rdquo; overruns the 330px box.']],
  }},
  {{
  ...
  {{
    ...
    desc: '7.5\'s collections index: the ONE landscape 16:9 tile in an app of 2:3 portraits, because a collection is a set rather than a title. Numbers are the spec's own, and the Android TV app lays its own out at exactly the same values.',
    states: [
...
[
  ['tofa_font_hero', 'Inter Tight Bold', 77, 700, 'Up', 'ONE caller: the server picker\u2019s heading. Measured EXACTLY right \u2014 its cap is 56px on the live app, and 77 \u00d7 Inter Tight\u2019s 0.7275 is 56.0'],
  ['tofa_font_hero_title', 'Inter Tight Bold', 61, 700, 'To the Journey: Looking Back', 'Home + Detail hero title, drawn only when a title has NO logo art. Split off tofa_font_hero 2026-08-19: 77 was 30% over the app there'],
